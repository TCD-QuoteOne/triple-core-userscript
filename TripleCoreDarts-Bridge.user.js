...
(function () {
    'use strict';
    const USERSCRIPT_VERSION = (typeof GM_info !== 'undefined' && GM_info?.script?.version) || '8.1.6';
    ...
    const Util = {
        ...
        getLobbyIdFromUrl() {
            ...
            const pathLobbyId = Util.normalizeLobbyId(pathMatch?.[1]);
            if (pathLobbyId) return pathLobbyId;

            const params = new URLSearchParams(location.search || '');
            const queryLobbyId = params.get('lobbyId') || params.get('lobby_id') || params.get('lobby');
            const normalizedQueryLobbyId = Util.normalizeLobbyId(queryLobbyId);
            if (normalizedQueryLobbyId) return normalizedQueryLobbyId;

            if (location.hash) {
                const hash = location.hash.startsWith('#') ? location.hash.slice(1) : location.hash;
                const hashPathMatch = hash.match(/\/lobbies\/([a-zA-Z0-9-]+)/i);
                const hashLobbyId = Util.normalizeLobbyId(hashPathMatch?.[1]);
                if (hashLobbyId) return hashLobbyId;
            }
    ...
    const ApiBridge = {
        async tcGet(path) {
            ...
            const discordGuildId = String(context?.discord_guild_id || context?.guild_id || '').trim();
            const headers = discordGuildId ? { 'X-Discord-Guild-ID': discordGuildId } : {};
        ...
        async tcPost(path, payload) {
            ...
            const response = await fetch(`${CONFIG.API_BASE}${path}`, {
                ...
                headers: {
                    ...
                    ...(discordGuildId ? { 'X-Discord-Guild-ID': discordGuildId } : {}),
                },
    ...
    const RouteAndIdentity = {
        ...
        hydrateSeasonContextFromUrl() {
            ...
            const discordGuildId = String(params.get('tc_discord_guild_id') || '').trim() || null;
        ...
        extractLobbyIdFromDom() {
            ...
            const parse = (raw) => {
                ...
                for (const candidate of [value, decoded]) {
                    ...
                    const lobbyId = Util.normalizeLobbyId(match?.[1]);
