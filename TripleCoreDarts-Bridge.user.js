...
(function () {
    ...
    function clearStaleLeagueContext(reason, context) {
    ...
    const LobbyFlow = {
        ...
        async handleCreatedLobby() {
            ...
            if (
                ...
                clearStaleLeagueContext('league_context_already_used_for_other_lobby', leagueContext);
            ...
            if (State.mode === 'league' && leagueContext && leagueContextMeta.expired) {
                clearStaleLeagueContext('league_context_ttl_exceeded_before_lobby_send', leagueContext);
            ...
            const payload = {
                ...
                season_match_id: State.activeSeasonMatchId || leagueJob?.season_match_id || ((State.mode === 'league' && leagueContextMeta.expired) ? null : context?.season_match_id) || null,
                season_id: leagueJob?.season_id || ((State.mode === 'league' && leagueContextMeta.expired) ? null : context?.season_id) || null,
                job_id: leagueJob?.id || casualJob?.id || ((State.mode === 'league' && leagueContextMeta.expired) ? null : context?.job_id) || null,
    ...
    async function refreshSeasonAndJobs() {
        ...
        try {
            ...
            if (leagueContextActive && contextMeta.expired) {
                ...
                Util.status('Liga-Kontext abgelaufen – bitte Match-Link erneut öffnen');
            } else if (!State.openLeagueJob && leagueContextActive) {
                Util.log('DEBUG context reset after league timeout/no-open-job', { context });
                Storage.clearContext();
                State.activeSeasonMatchId = null;
            }
        } catch (err) {
